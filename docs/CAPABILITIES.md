...
**Parse ebook files.** Shocking for an e-reader, I know. .ebook files are simply zipped archives, but Mímisbrunnur, as of even date, does not have the ability to deal with this. But you can preprocess them by unzipping them, for which various tools exist, including built in support in Windows and my favourite, [7zip](https://www.7-zip.org/). After that you can browse and read the contents.  
...
**Justified text.** It will be left aligned instead.  
...
**For UTF-16, only Unicode codepoints 0x00 to 0xFF will be correctly handled.** Not even the full CP1252. This is because of the extremely simplistic approach to UTF-16 decoding. In short, null characters are ignored, meaning that as long as the codepoint is <=0xFF it will treat the low-order byte as the complete codepoint. As you may surmise UTF-16 decoding has not been a priority.  
**Scripting** Whether JavaScript or any other, scripting of any kind is not supported, nor do I see it ever will be. This is partially because of the complexities it would entail, but even more because of security. Even with the best of intentions, any form of scripting would provide a vast attack surface, and I don't want to have to worry about that.  
**USB file system access** Currently you have to write your ebook to a micro-SD card using a separate card reader. It would be nice to have access to the file system through the readers USB connection, so it could be it's own card reader. This may happen yet.
