...
As mentioned above, this case started life as a display case for a Parker ballpoint pen and a miniature calculator. It is a felt covered stamped steel case, with a spring loaded hinge wich holds it in either closed or approx. 120° open position. It is about 18cm long, 11 cm wide, and 2.2cm thick (external measurements, excluding slight padding on the lid). I fitted a microswitch/limit switch that cuts battery power when the case is closed, eliminating any battery drain. While a case exactly like it may be hard to find, I think the basic layout has several advantages:
