...
The display is fixed on top of the baseplate with double sided tape, while the carrier board is fixed underneath the baseplate with screws and standoffs, and located such that the buttons come directly beneath the display, and the charge and general indicators are visible through the baseplate. The display cable is far too short to reach the controller when located like this, so an extention cable and interconnect are needed.
...
The reading light is fixed to the lip of the top half of the case with double sided tape.
